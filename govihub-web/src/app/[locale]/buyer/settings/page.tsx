...
const MOCK: UserProfile = { name:"Thilini Wickramasinghe", email:"thilini@example.com", phone:"+94712345678", district:"Colombo", language:"en", notifyMatches:true, notifyPrices:false, notifyAdvisory:false, notifySystem:true };
            ...
            <div className="space-y-2 mt-3">
              {[{value:"en",label:"English"},{value:"si",label:"සිංහල (Sinhala)"},{value:"ta",label:"தமிழ் (Tamil)"}].map(lang => (
                <label key={lang.value} className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="language" value={lang.value} checked={profile.language===lang.value} onChange={() => setField("language", lang.value)} className="w-4 h-4 text-amber-500" />

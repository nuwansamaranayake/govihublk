...
export default function FarmerSettingsPage() {
  ...
  const toggle = (key: keyof UserProfile) => {
    if (!profile) return;
    setProfile({ ...profile, [key]: !profile[key as keyof UserProfile] });
  };

  const setField = (key: keyof UserProfile, val: string) => {
    if (!profile) return;
  ...
  return (
    <div className="min-h-screen bg-neutral-50 pb-24">
      <div className="bg-green-700 px-4 pt-10 pb-6 text-white">
        ...
        <p className="text-green-200 text-sm mt-1">Manage your account preferences</p>
      </div>
        ...
        <form onSubmit={handleSave} className="px-4 py-4 space-y-4">
          ...
          <Card header={<h2 className="font-semibold text-neutral-800 text-sm">Profile Information</h2>} padding="md">
            <div className="space-y-3 mt-3">
              ...
              <Select label="District" value={profile.district} onChange={e => setField("district", e.target.value)}
            ...
            <div className="space-y-2 mt-3">
              {[{value:"en",label:"English"},{value:"si",label:"සිංහල (Sinhala)"},{value:"ta",label:"தமிழ் (Tamil)"}].map(lang => (
                <label key={lang.value} className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="language" value={lang.value} checked={profile.language===lang.value}
                    onChange={() => setField("language", lang.value)}
          ...
          <Card header={<h2 className="font-semibold text-neutral-800 text-sm">Notification Preferences</h2>} padding="md">
            <div className="space-y-3 mt-3">
              {[
                { key:"notifyMatches" as keyof UserProfile, label:"New match notifications" },
                { key:"notifyPrices" as keyof UserProfile, label:"Market price alerts" },
                { key:"notifyAdvisory" as keyof UserProfile, label:"Farm advisory updates" },
                ...
                <div key={item.key} className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors ${profile[item.key]?"bg-green-500":"bg-neutral-300"}`}
                  >
          ...
          <Card padding="md" className="border-red-200">
            ...
            <p className="text-sm text-neutral-500 mb-3">Deactivating your account will remove it from the marketplace. This action can be undone by contacting support.</p>
      ...
      <Modal isOpen={confirmDeactivate} onClose={() => setConfirmDeactivate(false)}
        title="Deactivate Account"
        footer={
          <div className="flex gap-3">
            <Button variant="ghost" fullWidth onClick={() => setConfirmDeactivate(false)}>Cancel</Button>
            <Button variant="danger" fullWidth onClick={handleDeactivate}>Yes, Deactivate</Button>
          </div>
        }
      >
        <p className="text-sm text-neutral-700">Are you sure you want to deactivate your account? Your listings and matches will be hidden until you reactivate.</p>
      </Modal>
    </div>
  );
}

...
def upgrade() -> None:
    ...
    # Enums
    user_role = postgresql.ENUM("farmer", "buyer", "supplier", "admin", name="user_role", create_type=False)
    crop_category = postgresql.ENUM("vegetable", "fruit", "grain", "pulse", "spice", name="crop_category", create_type=False)
    listing_status = postgresql.ENUM("draft", "active", "matched", "sold", "expired", "cancelled", name="listing_status", create_type=False)
    demand_status = postgresql.ENUM("draft", "active", "matched", "fulfilled", "expired", "cancelled", name="demand_status", create_type=False)
    match_status = postgresql.ENUM("proposed", "accepted_farmer", "accepted_buyer", "confirmed", "in_transit", "fulfilled", "disputed", "cancelled", "expired", name="match_status", create_type=False)
    diagnosis_status = postgresql.ENUM("pending", "processing", "completed", "failed", name="diagnosis_status", create_type=False)
    user_feedback_type = postgresql.ENUM("helpful", "not_helpful", "incorrect", name="user_feedback_type", create_type=False)
    supply_category = postgresql.ENUM("seeds", "fertilizer", "pesticide", "equipment", "tools", "irrigation", "other", name="supply_category", create_type=False)
    supply_status = postgresql.ENUM("active", "out_of_stock", "discontinued", name="supply_status", create_type=False)
    notification_type = postgresql.ENUM("match_found", "match_accepted", "match_confirmed", "match_fulfilled", "price_alert", "weather_alert", "diagnosis_complete", "system_message", name="notification_type", create_type=False)
    notification_channel = postgresql.ENUM("push", "sms", "in_app", name="notification_channel", create_type=False)
